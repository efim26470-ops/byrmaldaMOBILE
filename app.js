(function(){
  'use strict';
  const VERSION = 'mobile-clean-2.0.0';
  const SAVE_KEY = 'cs2_mobile_clean_save_v2';
  const CURRENCY = '₽LC';
  ...
  const REAL = {
    ...
    karambitDoppler:'https://cdn.csgoskins.gg/public/uih/products/aHR0cHM6Ly9jZG4uY3Nnb3NraW5zLmdnL3B1YmxpYy9pbWFnZXMvYnVja2V0cy9lY29uL2RlZmF1bHRfZ2VuZXJhdGVkL3dlYXBvbl9rbmlmZV9rYXJhbWJpdF9hbV9kb3BwbGVyX3BoYXNlMV9saWdodC4yYjdjYWFhMmE5OGRiNmY2NWU1MWJiMmQ5YThlNzA0MDhkNjAwOWE3LnBuZw--/auto/auto/85/notrim/bde86cbbe92dc9a7671a61764d6d82dd.webp',
  ...
  const caseCdn = p => IMG_BASE + p;
  const REAL_CASE_IMAGES = {
    kilowatt: REAL.kilowatt,
    revolution: caseCdn('weapon_cases/crate_community_31_png.png'),
    recoil: caseCdn('weapon_cases/crate_community_30_png.png'),
    dreams: caseCdn('weapon_cases/crate_community_29_png.png'),
    cheap: caseCdn('weapon_cases/crate_community_26_png.png'),
    red: caseCdn('weapon_cases/crate_community_17_png.png'),
    knife: REAL.karambitDoppler,
    gloves: caseCdn('weapon_cases/crate_glove_png.png'),
    stickers: caseCdn('stickers/community01/crate_sticker_pack_community01_png.png'),
    agents: caseCdn('characters/customplayer_tm_jumpsuit_variantb_png.png')
  };
  const REAL_ITEM_IMAGES = {
    'ak-redline': REAL.akRedline,
    'ak-vulcan': REAL.akRedline,
    'ak-empress': REAL.akRedline,
    'ak-head': REAL.akRedline,
    'awp-hyper': REAL.awpHyper,
    'awp-asiimov': REAL.awpHyper,
    'awp-duality': REAL.awpHyper,
    'awp-fever': REAL.awpHyper,
    'karambit-doppler': REAL.karambitDoppler,
    'karambit-gamma': REAL.karambitDoppler,
    'butterfly-doppler': REAL.karambitDoppler,
    'm9-fade': REAL.karambitDoppler
  };
  function realFallback(name='', type='rifle'){
    const n = String(name).toLowerCase();
    if(n.includes('awp')) return REAL.awpHyper;
    if(n.includes('knife') || n.includes('karambit') || n.includes('bayonet') || type==='knife') return REAL.karambitDoppler;
    if(n.includes('ak-47') || n.includes('m4') || n.includes('galil') || n.includes('famas')) return REAL.akRedline;
    if(type==='sticker' || type==='agent' || type==='patch' || type==='charm') return REAL.kilowatt;
    return REAL.akRedline;
  }
  function fixImageUrl(url){
    url = String(url || '').trim();
    if(!url) return '';
    const raw = 'https://raw.githubusercontent.com/ByMykel/counter-strike-image-tracker/main/';
    if(url.startsWith(raw)) return 'https://cdn.jsdelivr.net/gh/ByMykel/counter-strike-image-tracker@main/' + url.slice(raw.length);
    return url;
  }

  function weaponSvg(name, palette='red', type='rifle'){
    return realFallback(name, type);
  }
  function caseSvg(name, palette='orange'){
    const n = String(name||'').toLowerCase();
    if(n.includes('kilowatt')) return REAL_CASE_IMAGES.kilowatt;
    if(n.includes('revolution')) return REAL_CASE_IMAGES.revolution;
    if(n.includes('recoil')) return REAL_CASE_IMAGES.recoil;
    if(n.includes('dream')) return REAL_CASE_IMAGES.dreams;
    if(n.includes('budget') || n.includes('blue')) return REAL_CASE_IMAGES.cheap;
    if(n.includes('red') || n.includes('covert')) return REAL_CASE_IMAGES.red;
    if(n.includes('knife')) return REAL_CASE_IMAGES.knife;
    if(n.includes('glove')) return REAL_CASE_IMAGES.gloves;
    if(n.includes('sticker')) return REAL_CASE_IMAGES.stickers;
    if(n.includes('agent') || n.includes('charm')) return REAL_CASE_IMAGES.agents;
    return REAL_CASE_IMAGES.kilowatt;
  }
  const items = [
  ...
  function it(id,name,rarity,value,palette,type='rifle'){ return {id,name,rarity,value,rarityColor:rarityColor[rarity]||'#60a5fa',weight:weights[rarity]||10,image:REAL_ITEM_IMAGES[id]||weaponSvg(name,palette,type),type}; }
  const cases = [
  ...
  function cs(id,name,price,palette,pool,profit){ return {id,name,price,image:REAL_CASE_IMAGES[id]||caseSvg(name,palette),items:pool.length?pool:items,profit}; }

  async function hydrateRealImages(){
    // Не блокирует интерфейс: если CDN доступен, заменяет все SVG/резервные картинки на реальные изображения из открытого CS2-каталога.
    const endpoints = [
    ...
    let changed = false;
    items.forEach(item => {
      const raw = byName.get(String(item.name).toLowerCase());
      if(raw && raw.image){ item.image = fixImageUrl(raw.image); changed = true; }
    });
    if(changed){
      state.inventory.forEach(inv => {
        const src = items.find(x => x.name === inv.name);
        if(src && src.image) inv.image = src.image;
      });
      save();
      route();
    }
  ...
  function caseCard(c){ return `<article class="case-card"><div class="case-img-wrap"><img src="${c.image}" onerror="this.onerror=null;this.src='https://cdn.csgoskins.gg/public/uih/inspections/aHR0cHM6Ly9jZG4uY3Nnb3NraW5zLmdnL3B1YmxpYy9pbWFnZXMvaW5zcGVjdGlvbnMvdjIva2lsb3dhdHQtY2FzZS5wbmc-/auto/auto/85/notrim/d520932d9c745aceb17664cbb57b33bc.webp'" alt="${esc(c.name)}"></div><h3>${esc(c.name)}</h3><div class="case-meta"><span>${c.items.length} предметов</span><b class="price">${fmt(c.price)}</b></div><div class="chips">${[...new Set(c.items.map(i=>i.rarity))].slice(0,4).map(x=>`<span class="chip">${esc(x)}</span>`).join('')}</div><div class="case-actions"><button class="btn primary" type="button" data-action="open-case" data-case="${c.id}">Крутить</button><button class="btn" type="button" data-action="case-pool" data-case="${c.id}">Пул</button></div></article>`; }
  function itemCard(it, opts={}){ return `<article class="item-card ${opts.selected?'selected':''}" style="--rar:${it.rarityColor}" data-uid="${esc(it.uid||'')}"><div class="item-art"><img src="${it.image}" onerror="this.onerror=null;this.src='https://cdn.csgoskins.gg/public/uih/products/aHR0cHM6Ly9jZG4uY3Nnb3NraW5zLmdnL3B1YmxpYy9pbWFnZXMvYnVja2V0cy9lY29uL2RlZmF1bHRfZ2VuZXJhdGVkL3dlYXBvbl9hazQ3X2N1X2FrNDdfY29icmFfbGlnaHQuZjQ1OGMxMDVhYjc1ZmM3NGU4ZGVjMTlhZDU4YmJlZTkwNzJhMTE1YS5wbmc-/auto/auto/85/notrim/512a8c5e2da478fafdbd7648d0ccf5ba.webp'" alt="${esc(it.name)}"></div><h4>${esc(it.name)}</h4><small>${esc(it.rarity)}${it.wear?' · '+esc(it.wear):''}</small><div class="item-value"><b>${fmt(it.value)}</b>${opts.badge?`<span class="chip">${opts.badge}</span>`:''}</div>${opts.actions?`<div class="item-actions">${opts.actions}</div>`:''}</article>`; }
  ...
  function battle(id){ const c=cases.find(x=>x.id===id); if(!spend(c.price,'Battle '+c.name))return; const me=invItem(weighted(c)), bot=invItem(weighted(c)); state.battles++; let body=`<div class="results-grid"><div class="result-mini" style="--rar:${me.rarityColor}"><img src="${me.image}" onerror="this.onerror=null;this.src='https://cdn.csgoskins.gg/public/uih/products/aHR0cHM6Ly9jZG4uY3Nnb3NraW5zLmdnL3B1YmxpYy9pbWFnZXMvYnVja2V0cy9lY29uL2RlZmF1bHRfZ2VuZXJhdGVkL3dlYXBvbl9hazQ3X2N1X2FrNDdfY29icmFfbGlnaHQuZjQ1OGMxMDVhYjc1ZmM3NGU4ZGVjMTlhZDU4YmJlZTkwNzJhMTE1YS5wbmc-/auto/auto/85/notrim/512a8c5e2da478fafdbd7648d0ccf5ba.webp'"><b>Ты: ${esc(me.name)}</b><small>${fmt(me.value)}</small></div><div class="result-mini" style="--rar:${bot.rarityColor}"><img src="${bot.image}" onerror="this.onerror=null;this.src='https://cdn.csgoskins.gg/public/uih/products/aHR0cHM6Ly9jZG4uY3Nnb3NraW5zLmdnL3B1YmxpYy9pbWFnZXMvYnVja2V0cy9lY29uL2RlZmF1bHRfZ2VuZXJhdGVkL3dlYXBvbl9hazQ3X2N1X2FrNDdfY29icmFfbGlnaHQuZjQ1OGMxMDVhYjc1ZmM3NGU4ZGVjMTlhZDU4YmJlZTkwNzJhMTE1YS5wbmc-/auto/auto/85/notrim/512a8c5e2da478fafdbd7648d0ccf5ba.webp'"><b>Бот: ${esc(bot.name)}</b><small>${fmt(bot.value)}</small></div></div>`; if(me.value>=bot.value){ state.inventory.unshift(me,bot); state.wins++; body+=`<div class="notice">Победа! Оба предмета в инвентаре.</div>`; } else body+=`<div class="notice">Поражение. Предметы забрал бот.</div>`; save(); openModal('Case Battle', body+`<button class="btn primary" data-close-modal>Ок</button>`); }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{ if(!location.hash) location.hash='home'; route(); wireButtons(); setInterval(wireButtons,1000); setTimeout(hydrateRealImages, 150); });
})();
